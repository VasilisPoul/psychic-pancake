...
The `home-routes` handler in the `psychic-pancake.routes.home` namespace
...
The `home-page` function will in turn call the `psychic-pancake.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `psychic-pancake.handler` namespace:
...
Request middleware functions are located under the `psychic-pancake.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `psychic-pancake.dev-middleware` namespace found in
